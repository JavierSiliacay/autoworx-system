...
import { Printer, Calendar as CalendarIcon, FileDown, Eye, Edit, Save, Loader2, X } from "lucide-react"
...
export function ReleaseMonitoring({ records }: { records: any[] }) {
    ...
    const getCategorizedCosts = (costing: any) => {
        ...
        if (!costing || !costing.items) return result

        costing.items.forEach((item: any) => {
            const cat = item.category || ""
            const type = item.type || ""
            // Group body repair, tinsmith, detailing, painting
            if (
                cat === "Tinsmith/Alignment" ||
                cat === "Detailing" ||
                cat === "Painting" ||
                cat === "Glassworks" ||
                cat === "Remove and Install" ||
                type === "parts" || // Fallback if needed, but the prompt says 4 columns.
                cat === "Others"
            ) {
                // By default, if it's not the specific mechanical/electrical/aircon, we dump it into BRPAD since BRPAD represents Body Repair Painting And Detailing
                // If it's pure parts with no category, we will just add it here.
            }

            if (cat === "Aircon") {
                result.aircon += item.total || 0
            } else if (cat === "Electrical") {
                result.electrical += item.total || 0
            } else if (cat === "Mechanical Works") {
                result.mechanical += item.total || 0
            } else {
                result.brpad += item.total || 0
            }
        })
                ...
                <table className="w-full min-w-[1200px] border-collapse text-xs">
                    ...
                    <tbody>
                        {tableRecords.length > 0 ? (
                            ...
                            <tr>
                                <td colSpan={15} className="p-8 text-center text-muted-foreground italic border border-border">
