...
import {
  ...
  Filter, Layers, Save, X, Building2, Store
} from "lucide-react"
...
export default function OthersPriceListPage() {
  const router = useRouter()
  const { status } = useSession()

  const [items, setItems] = useState<PriceListOtherItem[]>([])
  ...
  const [editingItem, setEditingItem] = useState<PriceListOtherItem | null>(null)
  ...
  const fetchItems = async () => {
    setIsLoading(true)
    try {
      const res = await fetch("/api/parts/prices/others")
      if (res.ok) {
        const data = await res.json()
        setItems(data)
      } else {
        const errData = await res.json().catch(() => ({}))
        throw new Error(errData.error || "Failed to load others price list")
      }
    } catch (err: any) {
      console.error("Failed to fetch others items:", err)
      toast({ title: "Error", description: err.message || "Failed to load others price list.", variant: "destructive" })
    } finally {
      setIsLoading(false)
    }
  }
  ...
  const filteredItems = useMemo(() => {
    ...
    return items.filter(p => {
      ...
      // 2. Tokenized & Space-Insensitive Search Filter across item name, supplier name, category, unit
      if (searchTokens.length === 0) return matchesCategory

      const searchableText = `${p.item_name} ${p.supplier_name || ""} ${p.category} ${p.unit}`.toLowerCase()
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!finalCategory) {
        toast({ title: "Required Field", description: "Please enter a category name.", variant: "destructive" })
        setIsSaving(false)
        return
      }
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const res = await fetch(`/api/parts/prices/others?id=${id}`, { method: "DELETE" })
      if (res.ok) {
  ...
  const openAddModal = () => {
    ...
    setFormData({
      item_name: "",
      supplier_name: "",
      category: selectedCategory !== "All Categories" ? selectedCategory : "General Supplies",
      customCategory: "",
      unit: "PC",
      customUnit: "",
      supplier_price: "",
  ...
  const openEditModal = (item: PriceListOtherItem) => {
    const isStandardCategory = DEFAULT_CATEGORIES.includes(item.category)
  ...
  if (status === "loading" || isLoading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-slate-50">
        ...
        <p className="text-slate-500 font-medium">Loading Others Catalogue...</p>
      </div>
    )
  }
      ...
      <div className="min-h-screen bg-slate-50/50 pb-20">
          ...
          <div className="max-w-7xl mx-auto flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500 font-medium uppercase tracking-widest">Miscellaneous & Supplier Reference Catalogue</p>
              </div>
            </div>
        ...
        <main className="max-w-7xl mx-auto px-4 py-8 md:px-8">
          ...
          <div className="flex flex-wrap items-center gap-2 mb-8">
            ...
            {Array.from(new Set([...DEFAULT_CATEGORIES, ...activeCategories])).map(cat => (
              <Button
                key={cat}
                    ...
                    filteredItems.map((item) => (
                      <tr key={item.id} className="hover:bg-slate-50/80 transition-colors group">
                        ...
                        <td className="px-6 py-4">
                          {item.supplier_name ? (
                            <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md bg-amber-50 text-amber-800 border border-amber-200/60 font-semibold text-xs">
                              <Store className="w-3.5 h-3.5 text-amber-600" />
                              <span className="truncate max-w-[140px]" title={item.supplier_name}>
                                {item.supplier_name}
                              </span>
                            </div>
                          ) : (
                        ...
                        <td className="px-6 py-4 text-right">
                          <div className="flex flex-col items-end">
                            ...
                            <span className="text-[10px] text-slate-500 max-w-[110px] truncate" title={item.updated_by}>
                              {item.updated_by}
                            </span>
                          </div>
                        </td>
            ...
            <form onSubmit={handleSave}>
              ...
              <div className="space-y-4 py-5">
                <div className="grid gap-2">
                  <Label className="text-slate-700 font-medium">Item Name <span className="text-red-500">*</span></Label>
                  <Input
                    required
                    placeholder="e.g. Hex Bolt M8x30, Detailing Clay Bar, Windshield Sealant"
                ...
                <div className="grid gap-2">
                  <Label className="text-slate-700 font-medium">Supplier's Name</Label>
                  <div className="relative">
                    <Store className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
                    <Input
                      placeholder="e.g. Fastener Hub, Diamond Auto Supply"
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div className="grid gap-2">
                    <Label className="text-slate-700 font-medium">Unit <span className="text-red-500">*</span></Label>
                    <Select
                      value={formData.unit}
                ...
                {formData.category === "Custom" && (
                  <div className="grid gap-2">
                    <Label className="text-slate-700 font-medium">Custom Category <span className="text-red-500">*</span></Label>
                    <Input
                      required
                ...
                {formData.unit === "Custom" && (
                  <div className="grid gap-2">
                    <Label className="text-slate-700 font-medium">Custom Unit <span className="text-red-500">*</span></Label>
                    <Input
                      required
                      placeholder="e.g. roll, pack, yard"
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="grid gap-2">
                    <Label className="text-slate-700 font-medium">Supplier's Price (₱) <span className="text-red-500">*</span></Label>
                    <Input
                      required
                      placeholder="0.00"
